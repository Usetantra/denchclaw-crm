-- DenchClaw CRM — migration 012: channel compliance foundation (Phase A)
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_template_name_lang
  ON message_templates (company_id, channel, name, language);
CREATE INDEX IF NOT EXISTS idx_template_status ON message_templates (company_id, status);
...
CREATE INDEX IF NOT EXISTS idx_template_versions ON message_template_versions (template_id, version DESC);
...
ALTER TABLE messages ADD COLUMN IF NOT EXISTS template_id      UUID;          -- FK-ish → message_templates.id
