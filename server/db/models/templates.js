...
// Meta. Every edit snapshots the prior definition into message_template_versions.
...
async function list(companyId, { channel, status, includeArchived = false } = {}) {
  ...
  const r = await query(`SELECT ${FIELDS} FROM message_templates WHERE ${where} ORDER BY updated_at DESC`, params);
  return r.rows;
}

async function get(companyId, id) {
  const r = await query(`SELECT ${FIELDS} FROM message_templates WHERE company_id=$1 AND id=$2 LIMIT 1`, [companyId, id]);
...
async function update(companyId, id, patch) {
  ...
  if (!cur) return null;
  await query(
    `INSERT INTO message_template_versions (template_id, company_id, version, snapshot, status)
     VALUES ($1,$2,$3,$4,$5)`,
  ...
  const r = await query(
    `UPDATE message_templates SET name=$1, language=$2, category=$3, header=$4, body=$5, footer=$6,
...
async function setProvider(companyId, id, { providerTemplateId, status, submittedCategory }) {
  const r = await query(
    `UPDATE message_templates SET provider_template_id=COALESCE($1, provider_template_id),
...
async function syncStatus(companyId, id, { status, currentCategory, rejectionReason }) {
  const r = await query(
    `UPDATE message_templates SET status=COALESCE($1,status), current_category=COALESCE($2,current_category),
...
async function archive(companyId, id) {
  const r = await query(`UPDATE message_templates SET status='ARCHIVED', updated_at=now() WHERE company_id=$1 AND id=$2 RETURNING ${FIELDS}`, [companyId, id]);
  return r.rows[0] || null;
}

async function remove(companyId, id) {
  await query(`DELETE FROM message_templates WHERE company_id=$1 AND id=$2`, [companyId, id]);
  return true;
}

async function versions(companyId, id) {
  const r = await query(`SELECT version, status, snapshot, created_at FROM message_template_versions WHERE company_id=$1 AND template_id=$2 ORDER BY version DESC`, [companyId, id]);
  return r.rows;
}
